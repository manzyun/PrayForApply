...
func _on_main_generate_applyer_graphics(status):
    ...
    set_applyer_license(status["license"])
    set_applyer_apeal(status["apeal"])
    set_applyer_preferred_department(status["occupation"])
    pass # Replace with function body.


func _on_Applyer_graphics_pare(var body, var head):
    $ApplyerPhoto/Applyer.animated_sprite_body.frame = body
    $ApplyerPhoto/Applyer.animated_sprite_head.frame = head
    pass
